...
class MPU:
    ...
    def _thread_handler(cls) -> None:
        while cls._status:
            try:
                ...
                block_data = cls._gyro_acc.read_block_data(HW290.REGISTER.STATUS, 1)

                if not block_data[0] & 0x01:
                    continue

                # Accel data
                block_data = cls._gyro_acc.read_block_data(HW290.REGISTER.ACCEL_DATA, 6)
                ...
                # Gyro data
                block_data = cls._gyro_acc.read_block_data(HW290.REGISTER.GYRO_DATA, 6)
    ...
    def start(self) -> None:
        ...
        self._gyro_acc.write_byte(HW290.REGISTER.PWR_MGMT_1, 0x00)

        # Accel Sensitivity
        # 0, 8, 16 and 24 (int) for 16384, 8192, 4096 and 2048 sensitivity respectively
        self._gyro_acc.write_byte(HW290.REGISTER.ACCEL_SENSITIVITY, 0x18)

        # Gyro Sensitivity
        # 0, 8, 16 and 24 (int) for 131, 65.5, 32.8 and 16.4 sensitivity respectively
        self._gyro_acc.write_byte(HW290.REGISTER.GYRO_SENSITIVITY, 0x18)
